...
impl Worker {
    ...
    pub fn set_clusters(&mut self, k: usize) {
        ...
        if k > self.data_set.len() {
            ...
            panic!(
                "k ({}) can NOT be greater than the total rows ({}) in the data set!",
                k,
                self.data_set.len()
            );
        }
    ...
    fn choose_centroids(&mut self, k: usize) {
        ...
        loop {
            // until the desired k unique randoms are picked
            let random_selection = domain.ind_sample(&mut rng);
            if randoms.contains(&random_selection) {
                continue;
            }
        ...
        for i in 0..k {
            ...
            if err {
                panic!("Error getting data index from id {}", rand_id);
            }
            let m = Mean {
